...
<head>
    ...
    <script>
        var headers;
        let titleField, titleDiv, selectPlace, selectDiv, textArea, counterDescription, counter, counterMax, spanDiv,
            startDateField, stopDateField, startDateDiv, stopDateDiv, userConsultedId, includeFile, description,
            errDescription, infoNbChar, txtLength, maxLength, prec, isLimitReached, isStartDateValid, isJustValidateActive, isJustValidate;
        $(function() {
            titleDiv = $("#titlePart");
            titleField = $("#title");
        ...
        function getIteration3(maxCharacter) {
            ...
            const validation = new JustValidate("#add_experience_validation", {
        ...
        function callEventsToCheckValidation() {
            ...
            $("#add_experience_validation").on('submit', function(e) {
                return checkAll();
            });
        }
...
<body>
    ...
    <div class="main">
        <?php
        $url_add_experience = "experience/add_experience_to_user";
        $url_cancel = "experience/experiences";
        if ($user_consulted && $user->is_admin() && $user_consulted->id !== $user->id) {
            $url_add_experience = "experience/add_experience_to_user/$user_consulted->id";
            $url_cancel = "experience/experiences/$user_consulted->id";
            $user = $user_consulted;
        }
        ?>

        <h1>Add a new Experience for <?= $user->fullname ?></h1>
        <form id="add_experience_validation" action=<?= $url_add_experience ?> method="post">
            <div id="start-div" class="mb-3">
                <label for="start_date" class="form-label">Start date :</label>
                <input type="date" class="form-control" id="start_date" name="start" value="<?= $experience->start ?>">
            </div>

            <div id="stop-div" class="mb-3">
                <label for="stop_date" class="form-label">End date (optional) :</label>
                <input type="date" class="form-control" id="stop_date" name="stop" value="<?= $experience->stop ?>">
            </div>
            ...
            <div id="selectDiv" class="mb-3">
                ...
                <select id="selectPlace" class="form-select" name="selected-place">
                    <?php foreach ($places as $place) : ?>
                        <?php if ($experience->place && $experience->place->id == $place->id) : ?>
            ...
            <div class="mb-3 manage-buttons">
                <a href="<?= $url_cancel ?>" role="button" class="btn btn-secondary btn-cancel">Cancel</a>
                <input id="button-submit" type="submit" value="Add" class="btn btn-primary btn-add">
            </div>
        </form>
